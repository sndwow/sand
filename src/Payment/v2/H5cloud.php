 ...
 * 电子钱包【云账户】
 */
class H5cloud extends AbstractGateway
{
    public function orderCreate(array $params)
    {
        try {

            return 'https://faspay-oss.sandpay.com.cn/pay/h5/cloud?' . http_build_query($this->h5struct($params)); // 返回支付url

        }catch (\Throwable $e) {
            $newException = $e instanceof SandException ? $e : new BusinessException(
                json_encode(['电子钱包【云账户】','message' => $e->getMessage(),'file' => $e->getFile(),'line' => $e->getLine()]),
                $this,
                $e
            );
            throw $newException;
    ...
    public function orderRefund(array $body)
    {
        try {
            ...
            $this->errTraceName = 'H5cloud--orderRefund';
    ...
    public function orderQuery(array $body)
    {
        try {
            ...
            $this->errTraceName = 'H5cloud--orderQuery';
    ...
    public function orderMcAutoNotice(array $body)
    {
        try {
            ...
            $this->errTraceName = 'H5cloud--orderMcAutoNotice';
